...
export function activate(context: vscode.ExtensionContext) {
    ...
    let tasksCache: any[] = [];
    let lastProjectId: number | undefined = -2;

    async function updateTasksCache() {
        try {
            const projectId = context.workspaceState.get<number>('taskflow.projectId');
    ...
    let commitProvider = vscode.languages.registerCompletionItemProvider(
        [{ language: 'git-commit' }, { scheme: 'vscode-scm' }],
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                if (!apiService.hasToken()) return undefined;
                
                const linePrefix = document.lineAt(position).text.substr(0, position.character);
                const lastHashIndex = linePrefix.lastIndexOf('#');
                if (lastHashIndex === -1) return undefined;
                ...
                if (textAfterHash.length > 0 && !/^\d*$/.test(textAfterHash)) return undefined;
                if (textAfterHash.includes(' ')) return undefined;
...
class TaskFlowSidebarProvider implements vscode.WebviewViewProvider {
            ...
            switch (data.type) {
                case 'updateStatus': {
                    ...
                    try {
                        ...
                        if (err instanceof ApiServiceError && err.code === 'UNAUTHORIZED') {
                            vscode.window.showErrorMessage('TaskFlow: Session expired. Please re-authenticate.');
                        } else if (err instanceof ApiServiceError && err.code === 'NETWORK') {
    ...
    public async refresh() {
        ...
        const currentProjectId = this._context.workspaceState.get<number>('taskflow.projectId');
        ...
        try {
            ...
            } else if (err instanceof ApiServiceError && err.code === 'NOT_FOUND') {
    ...
    private _getHtmlForError(title: string, message: string, showAuth = false) {
    ...
    private _getHtmlForTasks(tasks: any[], projects: any[], currentProjectId?: number) {
        let taskItems = '';
        if (tasks.length === 0) {
            taskItems = `
